...
class MetricReportsIndividualSalonByStaff1 extends REST_Controller
{
    /**
    AUTHOR: Subbu
    ...
    **/
    ...
    CONST SERVICE_URL_TODAY = 'MetricReportsIndividualSalonByStaff/getMetricReportsIndividualSalonByStaff/Today';
    CONST SERVICE_URL_LASTWEEK = 'MetricReportsIndividualSalonByStaff/getMetricReportsIndividualSalonByStaff/Lastweek';
    CONST SERVICE_URL_LASTMONTH = 'MetricReportsIndividualSalonByStaff/getMetricReportsIndividualSalonByStaff/Lastmonth';
    CONST SERVICE_URL_LAST90DAYS = 'MetricReportsIndividualSalonByStaff/getMetricReportsIndividualSalonByStaff/Last90days';
     ...
     */
    function getMetricReportsIndividualSalonByStaff()
        { 
           if(isset($_POST['salon_id'])){
            ...
            if(isset($_POST['salon_id']) && $_POST['salon_id']!=''){
                ...
                if(!empty($salon) && isset($salon["millennium_enabled"]) && isset($salon["service_retail_reports_enabled"]) && $salon["millennium_enabled"]=="Yes" && $salon["service_retail_reports_enabled"]=="Yes"){
                     ...
                     $dbrange = $this->Range;
                    // Current Year Data
                    $whereCondition = array('salon_id' => $salon_id,'staff_id' => $staff_id,'dayRangeType' => $dbrange,'start_date' => $this->startDate,'end_date' => $this->endDate);
                    $res1 = $this->db->get_where(MILL_REPORT_CALCULATIONS_CRON,$whereCondition)->row_array();
                    //pa($this->db->last_query());
                    //pa($res1,'res1',false);
                    // Last Year Data
                    $whereCondition = array('salon_id' => $salon_id,'staff_id'=>$staff_id,'dayRangeType' => $dbrange,'start_date' => $this->lastYearStartDate,'end_date' => $this->lastYearEndDate);

                    $res2 = $this->db->get_where(MILL_REPORT_CALCULATIONS_CRON,$whereCondition)->row_array();
                   ...
                   // $ownerdata['dateRange'] = $this->startDate . " to " . $this->endDate;
                    ...
                    $res[4]['key'] = 'new_guest_qty';
                    $res[4]['type'] = '';
                    $res[5]['title'] = 'Repeat Guest'; 
                    $res[5]['key'] = 'repeated_guest_qty';
                    $res[5]['type'] = '';
                    $res[6]['title'] = 'RPCT'; 
                    $res[6]['key'] = 'RPCT';
                    $res[6]['type'] = '$';
                    ...
                    foreach ($res as $key => $value) {
                            ...
                            $metrics_data['value'] = !empty($res1) ?  $res1[$value['key']] : '0.0';
                            $metrics_data['value_ly'] = !empty($res2) ? $res2[$value['key']] : '0.0';
