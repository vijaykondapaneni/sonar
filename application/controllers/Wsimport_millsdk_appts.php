...
class Wsimport_millsdk_appts extends CI_Controller {
	                     ...
	                     if(isset($this->millResponseXml['Apptointments']))
								{
			                           ...
			                           // pa($this->millResponseXml['Apptointments'],'data',true);
										...
										//CONVERTED APPOINTMENTS XML TO ARRAY
										{
                                            ...
                                            if(($dayRangeType!='Oneday') && ($dayRangeType!='Oneweek')){
										...
										$this->getMissedClients($account_no);	
    ...
    */
    public function appointmentsCheckedIn($account_no="",$startDate="",$endDate=""){
		...
		if($getConfigDetails->num_rows()>0){
		 	foreach($getConfigDetails->result_array() as $configDetails){
		 		pa('',"".$configDetails['salon_name'].' ---['.$configDetails['salon_account_id']."]");
                    ...
                    if($this->millResponseSessionId){
	                    ...
	                    if(isset($this->millResponseXml['Apptointments'])){
						   ...
						   //print_r($postDataArr);exit;
							...
							if(!empty($postDataArr["checked_in_appts"])){
                    ...
                    //LogOff Session
                    $millMethodParams = array();
		            $this->millLogOffResponseXml = $this->nusoap_library->getMillMethodCall('LogOff',$millMethodParams);
		            echo $this->millLogOffResponseXml['LogOffResult'];echo "<br>";
		            //var_dump($logOffResponse);echo "<br>";;
		            if(!empty($this->millLogOffResponseXml['LogOffResult']=='true')){
		            	echo "Session Logged Out";echo "<br>";
		            } else {
		            	echo "Session Not Logged Out";echo "<br>";
		            }
        ...
        public function getMissedClients($account_no){
          if(!empty($account_no)){
          	$currentDate = date("Y-m-d");
          	$missedclients = $this->db->query("SELECT DISTINCT ClientId,AccountNo FROM mill_appointments WHERE AccountNo=".$account_no." and ClientId!=-999 AND SlcStatus!='Deleted' and str_to_date(AppointmentDate, '%m/%d/%Y') >= '".$currentDate."' AND ClientId NOT IN (SELECT Clientid FROM mill_clients where AccountNo=".$account_no.") limit 100")->result_array();

          	if(!empty($missedclients)){
          		foreach ($missedclients as $key => $value) {
          			$client_id = $value['ClientId'];
          			if(!empty($client_id)){
                       $this->updateClientInfo($client_id,$account_no);
          			}
          		}
          	}else{
          		pa("All Clients Matched");
          	}
          }
    }

    public function getUpdatedClients($account_no){
    	if(!empty($account_no)){
	        ...
	        if($this->millResponseSessionId){
	    		...
	    		$allclients = $this->db->query("SELECT ClientId,AccountNo FROM mill_appointments where AccountNo = '".$account_no."' and SlcStatus != 'Deleted' and ClientId != '-999' and str_to_date(AppointmentDate, '%m/%d/%Y') >= '".$start_date."' and str_to_date(AppointmentDate, '%m/%d/%Y') <= '".$end_date."'")->result_array();
    ...
    public function updateClientInfo($Clientid,$account_no){
    	pa($Clientid,'Clients Table Modifications',false);
    	$getConfigDetails = $this->Common_model->getMillSdkConfigDetails($account_no);
    	$configDetails = $getConfigDetails->row_array();
        $client_id = $Clientid;
    	$account_no = $this->salonAccountId = $configDetails['salon_account_id'];
        ...
        pa($this->millResponseSessionId,'Session',false);
        if($this->millResponseSessionId){
        	$millMethodParams = array('ClientId' => $Clientid);
	     	pa($millMethodParams,'params');
	        $this->millClientResponseXml = $this->nusoap_library->getMillMethodCall('GetClient',$millMethodParams);
	        pa($this->millClientResponseXml['GetClientResult']['Id'],'ClientIdTest');
	        ...
	        if(isset($this->millClientResponseXml['GetClientResult']['Id'])){
		        ...
		        $whereConditions =  array('ClientId' => $this->millClientResponseXml['GetClientResult']['Id'],'AccountNo' => $account_no);												
			    $arrClientsdata = $this->Twoyearclientsimport_model
							                             ->compareMillTwoYearsClients($whereConditions)
							                             ->row_array(); 
				$firstVisitDate = (!empty($this->millClientResponseXml['GetClientResult']['FirstVisitDate'])) ? str_replace("T"," ",$this->millClientResponseXml['GetClientResult']['FirstVisitDate']) : '';
				...
				$account_no_array =  array(827770827,1242022908,890474984,1098156639,1980840379);
				//DEPIL BRAZIL OPTIN ALL FOR ALL LOCATIONS
				if(in_array($account_no,$account_no_array)){
					$comparearray['opted_in_email'] = 1;
					$comparearray['opted_in_sms'] = 1;
				} else {
					$comparearray['opted_in_email'] = $optedInEMail;
					$comparearray['opted_in_sms'] = $optedInSms;
				}
				...
				if(!empty($arrClientsdata))
	                {
	                   ...
	                   if(empty($diff_array))
	                    ...
	                    }else
	                    {
	                        ...
	                        $whereconditions = array();
							$whereconditions['ClientId'] = $this->millClientResponseXml['GetClientResult']['Id'];
							$whereconditions['AccountNo'] = $account_no;
							//$this->Twoyearclientsimport_model->updateMillTwoYearsClients($whereconditions,$diff_array);
