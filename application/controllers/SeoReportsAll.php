...
class SeoReportsAll extends CI_Controller {
    ...
    public function getNewReports($account_no,$start_date,$end_date){
    	...
    	if(empty($_SESSION['seosessionnew'])){
            $this->load->view('checkseologinnew');
    	}else{
	       ...
	       $get = $this->db->query("SELECT * FROM 
                        ...
                        str_to_date(AppointmentDate, '%m/%d/%Y') <= '".$end_date."' group by ApptId order by AppointmentDate,AppointmentTime asc
                        ")->result_array();
    	...
    	$this->load->view('seoreportsnewall',$data);
      }	
    }
