<?php
	...
	class Wsexport_mill_data_dm extends CI_Controller {
		...
		function exporting_mill_appointments() {
			...
			foreach($get_account_nos->result_array() as $account_nos) {
				...
				$this->DB_ReadOnly->where(array("AccountNo" => $account_no, "IsProcessedDM" => 0));
				...
				if($get_appointments->num_rows() > 0) {
					...
					$ch = curl_init();
					curl_setopt($ch, CURLOPT_URL,"http://dm1.salonclouds.io/app/index.php/wsimport_data_new/importing_mill_appointments_sdk/".$account_no);
					...
					if($response['IsTotalSuccess'] == true) {
						...
						$this->db->where(array("AccountNo" => $account_no, "IsProcessedDM" => 0));
						$update_array = array(
							 "IsProcessedDM" => 1
						);
		...
		function exporting_mill_clients() {
			...
			foreach($get_account_nos->result_array() as $account_nos) {
				...
				$account_no = $account_nos['AccountNo'];
				$this->DB_ReadOnly->limit(500);
				$this->DB_ReadOnly->where(array("AccountNo" => $account_no, "IsProcessedDM" => 0, "ClientId!=" => ''));
				...
				if($get_appointments->num_rows() > 0) {
					...
					$post_body = json_encode($data);
					$ch = curl_init();
					curl_setopt($ch, CURLOPT_URL,"http://dm1.salonclouds.io/app/index.php/wsimport_data_new/importing_mill_clients_sdk/".$account_no);
					curl_setopt($ch, CURLOPT_RETURNTRANSFER,1);
					curl_setopt($ch, CURLOPT_POST, 1);
					curl_setopt($ch, CURLOPT_POSTFIELDS, $post_body); 
					curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json')); 
