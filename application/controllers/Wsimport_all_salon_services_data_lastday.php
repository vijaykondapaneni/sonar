...
class Wsimport_all_salon_services_data_lastday extends CI_Controller {
   ...
   function __construct() {
     ...
     */
	function GetServiceSales($salon_code_encode="")
		{
			...
			if($getConfigDetails->num_rows()>0)
			{
				foreach($getConfigDetails->result_array() as $configDetails)
				{
                    ...
                    // $log['CronUrl'] = 'http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
                     $log['CronUrl'] = MAIN_SERVER_URL.'Wsimport_all_salon_services_data_lastday/GetServiceSales/'.$salon_code_encode;
                     $log['CronType'] = 0;
                     $log['id'] = 0;
					...
					if($this->millResponseSessionId){
					 ...
					 if(isset($this->millResponseXml['ServiceSales']))
					  {
							$serviceSalesCount = count($millResponseXml['ServiceSales']);
							...
							foreach($this->millResponseXml['ServiceSales'] as $sSales) 
							  {
								...
								else // INSERT APPOINTMENT DATA IN DB 
								{
		                            ...
		                            $sSales['insert_status'] = self::INSERTED;
