...
class MetricReportsStaffIndividualSalon extends REST_Controller
{
    /**
    AUTHOR: Subbu
    ...
    public function __getType($metric){
        ...
        switch ($metric) {
            ...
            case 'color_percentage':
               $this->column = 'color_percentage';
                break;
            case 'avg_service_ticket':
               $this->column = 'avg_service_ticket';
                break;
            case 'estimated_sales':
               $this->column = 'estimated_sales';
                break;            
            default:
                $this->column = 'total_service_sales';
                break;
    ...
    */
    ...
    public function getMetricReportsStaffIndividualSalon(){
        ...
        if(!empty($salon) && isset($salon["millennium_enabled"]) && isset($salon["service_retail_reports_enabled"]) && $salon["millennium_enabled"]=="Yes" && $salon["service_retail_reports_enabled"]=="Yes"){
              ...
              $res1 = $this->db->query('SELECT reports.salon_id,start_date,end_date,reports.'.$this->column.',emp.name,emp.emp_iid,emp.image,emp.staff_id  FROM 
                    '.MILL_REPORT_CALCULATIONS_CRON.' reports 
                    join '.STAFF2_TABLE.' emp on reports.staff_id=emp.staff_id
                    WHERE 
                    reports.salon_id = "'.$salon_id.'" and 
                    reports.start_date = "'.$this->startDate.'" and
                    reports.end_date = "'.$this->endDate.'" and
                    reports.dayRangeType = "'.$this->Range.'" group by reports.staff_id order by emp.name asc')->result_array();
