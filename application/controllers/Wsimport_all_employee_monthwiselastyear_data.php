...
class Wsimport_all_employee_monthwiselastyear_data extends CI_Controller
{
    /**
    AUTHOR: Subbu
    ...
    **/
    ...
    function __construct()
    {
        ...
        $this->load->model('Employeeimportmonthwiselastyear_model');
    ...
    function __getStartEndDate($dayRangeType, $numOfYears)
    {
        ...
        switch ($this->dayRangeType) {
            case IndividualMonth:
                ...
                print $this->startDate = date('Y-m-d',strtotime($startDate." -$numOfYears year"));
                print $this->endDate = date('Y-m-d', strtotime($endDate." -$numOfYears year"));
            break;
            default:
     ...
     */
    function GetEmployeeScheduleHoursForMonthWiseLastYear($dayRangeType="",$account_no="",$numOfYears=1)
        {
            if($account_no!=''){
            ...
            if($getConfigDetails->num_rows()>0)
            {
                foreach($getConfigDetails->result_array() as $configDetails)
                {
                    ...
                    $log['CronUrl'] = MAIN_SERVER_URL.'Wsimport_all_employee_monthwiselastyear_data/GetEmployeeScheduleHoursForMonthWiseLastYear/'.$dayRangeType.'/'.$account_no;
                    $log['CronType'] = 0;
                    $log['id'] = 0;
                                ...
                                if ($this->millResponseXml){
                                        ...
                                        if(isset($this->millResponseXml['EmployeeScheduleHours']))
                                        {
                                            ...
                                            foreach($arrEmployeeScheduleHours as $emlist)
                                            {
                                                ...
                                                $arrEmployeelist = $this->Employeeimportmonthwiselastyear_model
                                                                ->compareMillEmployeesScheduleHoursMonthWiseLastYear($whereCondition)
                                                                ->row_array();
                                                ...
                                                if(!empty($arrEmployeelist))
                                                {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        $res = $this->Employeeimportmonthwiselastyear_model->updateEmployeesScheduleHoursMonthWiseLastYear($whereconditions,$diff_array);
                                                    }
                                                }
                                                else // INSERT APPOINTMENT DATA IN DB 
                                                {
                                                   ...
                                                   $emlist['insert_status'] = self::INSERTED;
                                                    ...
                                                    $res = $this->Employeeimportmonthwiselastyear_model->insertMillEmployeesScheduleHoursMonthWiseLastYear($emlist);
