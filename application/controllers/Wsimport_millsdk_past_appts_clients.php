...
class Wsimport_millsdk_past_appts_clients extends CI_Controller {
   ...
   function __construct() {
     ...
     */
    function getPastAppointmentsClients($startDate="",$endDate="",$account_no="")
		{
			...
			if($getConfigDetails->num_rows()>0)
			{
				...
				foreach($getConfigDetails->result_array() as $configDetails)
				{
                   ...
                   // pa($millMethodParams,'millMethodParams',true);
					...
					{
						$clientsCount = count($this->millResponseXml['Apptointments']);
						pa('',"The total Clients in SDK".$clientsCount,false); //DEBUGMSG
						foreach($this->millResponseXml['Apptointments'] as $mClients) //CONVERTED Clients XML TO ARRAY
						{	
							pa('',"Clients Id--".$mClients["iclientid"],false); //DEBUGMSG
							$client_id = $mClients["iclientid"];
							...
							//CHECKING DUPLICATION OF CLIENT ID AND 
							 {
                               ...
                               // pa($this->db->last_query(),"Last Query");
								...
								$optedInEMail = (!empty($this->millClientResponseXml['GetClientResult']['ConfirmViaEmail'])&& $this->millClientResponseXml['GetClientResult']['ConfirmViaSMS']=='true') ? 1:2;
                                   ...
                                   pa('',"Clients Service--".$configDetails['salon_name']."".$this->millClientResponseXml['GetClientResult']['Id'],false);
                                       ...
                                       if(empty($diff_array))
			                            {
                                            ...
                                            $diff_array['SlcStatus'] = Wsimport_millsdk_past_appts_clients::UPDATED;
			                        ...
			                        }else
										{
											...
											$comparearray['SlcStatus'] = Wsimport_millsdk_past_appts_clients::INSERTED;
