...
class CheckNumbersFromSdkCron extends CI_Controller
{
    /**
    AUTHOR: Subbu
    ...
    **/
     ...
     */
    function getCheckNumbersFromSdkCron($dayRangeType="today",$salon_id="")
        { 
                        ...
                        if($get_session_status['session_status']==0){
                           ...
                           $millloginDetails = array('User' => $this->salonMillUsername,'Password' => $this->salonMillPassword);
                            ...
                            if($this->millResponseSessionId){
                                ...
                                pa($service_total_sales,'sdk_service_total_sales',false);
                                if($service_total_sales!= $res1['service_revenue']) {
                                    print "Service Not Matched";
                                    $value = 'NumbersMatch/Services';
                                    $url = $mainurl.$value.'/'.$salon_id.'/'.$this->startDate.'/'.$this->endDate.'/lastweek';
                                    $op = $this->Common_model->insertDataByUsingCUrl($url);
                                }else{
                                    $value = 'NumbersMatch/Services';
                                    $op = $this->Common_model->insertDataByUsingCUrl($url);
                                     print "Service Matched";
                                }
                                ...
                                pa($product_total_sales,'sdk_product_total_sales',false);
                                if($product_total_sales!= $res1['total_retail_price']) {
                                    print "Product Not Matched";
                                    $value = 'NumbersMatch/Products';
                                    $url = $mainurl.$value.'/'.$salon_id.'/'.$this->startDate.'/'.$this->endDate.'/lastweek';
                                    $op = $this->Common_model->insertDataByUsingCUrl($url);
                                }else{
                                ...
                                pa($GC_total_Sales,'sdk_GC_total_Sales',false);
                                if($GC_total_Sales!= $res1['gift_cards']) {
                                    print "Gift Card Not Matched";
                                    $value = 'NumbersMatch/GiftCards';
                                    $url = $mainurl.$value.'/'.$salon_id.'/'.$this->startDate.'/'.$this->endDate.'/lastweek';
                                    $op = $this->Common_model->insertDataByUsingCUrl($url);
                                }else{
                                     print "Gift Card Matched";
                                }
                              }    
