...
class Wsimport_millsdk_appts_new extends CI_Controller {
    /**
       AUTHOR: Subbu
                                       ...
                                       pa($countOfAppts,'SDKCOUNT',false);
                                        ...
                                        //CONVERTED APPOINTMENTS XML TO ARRAY
                                        {
                                            ...
                                            if(!in_array($client_id,$clientsIds) && $client_id != "-999"){
                                                $clientsIdsArr[] = $appts['iclientid'];
                                            }
                                            
                                        }
                                        //echo "<pre>";
                                        // print_r($clientsIdsArr);
                                        $uniqClientIds = array_unique($clientsIdsArr);
                                        $xmlIDS = '';
                                        foreach ($uniqClientIds as $key => $uClientId) {
                                            $xmlIDS .='<Ids><Id>'.$uClientId.'</Id></Ids>,';     
                                        }
                                        $error_array = '';
                                        $trimedClientIds = rtrim($xmlIDS,",");
                                        if(!empty($trimedClientIds)){
                                            $millMethodParams = array('XmlIds' => '<NewDataSet>'.$trimedClientIds.'</NewDataSet>');
                                            //pa($millMethodParams,'params');
                                            $this->millClientResponseXml = $this->nusoap_library->getMillMethodCall('GetClientInfo',$millMethodParams);
                                            // print_r($this->millClientResponseXml['Clients']);
                                            //pa($this->millClientResponseXml['GetClientInfoResult']['Id'],'ClientIdTest');

                                            //pa($this->millClientResponseXml,'response');
                                            foreach ($this->millClientResponseXml['Clients'] as $key => $clientValue) {
                                                
                                                if(!isset($clientValue['iid'])){
                                                    $error_client_ids[] = $client_id;
                                                    $error_array = array_unique($error_client_ids);
                                                    continue;
                                                }
                                                pa($error_array,'error_client_ids');
                                                //print_r($clientValue);
                                                $whereConditions =  array('ClientId' => $clientValue['iid'],'AccountNo' => $this->salonAccountId);
                                            
                                                $arrClientsdata = $this->Twoyearclientsimport_model
                                                     ->compareMillTwoYearsClients($whereConditions)
                                                     ->row_array();
                                                $mobilePhoneNo = (!empty($clientValue['ccellphone'])) ? $clientValue['ccellphone']:0;
                                                $firstVisitDate = (!empty($clientValue['dfirstvisit'])) ? str_replace("T"," ",$clientValue['dfirstvisit']) : '';

                                                $lastVisitDate = (!empty($clientValue['dlastvisit']) ? str_replace("T"," ",$clientValue['dlastvisit']) : '');

                                                //$optedInSms = (!empty($clientValue['lsmsblast'])&& $clientValue['lsmsblast']=='true') ? 1:2;

                                                //$optedInEMail = (!empty($clientValue['lnomail'])&& $clientValue['lnomail']=='true') ? 1:2;

                                                $HasMemberships = 0;//(!empty($this->millClientResponseXml['GetClientResult']['HasMemberships'])&& $this->millClientResponseXml['GetClientResult']['HasMemberships']=='true') ? 1:0;

                                                $cemail = (is_array($clientValue['cemail']) ? '' : trim($clientValue['cemail']));
                                                $czip = (is_array($clientValue['czip']) ? '' : trim($clientValue['czip']));
                                                $carea = (is_array($clientValue['carea']) ? '' : trim($clientValue['carea']));
                                                $chomephone = (is_array($clientValue['chomephone']) ? '' : trim($clientValue['chomephone']));
                                                $cfirstname = (is_array($clientValue['cfirstname']) ? '' : trim($clientValue['cfirstname']));
                                                $clastname = (is_array($clientValue['clastname']) ? '' : trim($clientValue['clastname']));
                                                $dbday =  str_replace("T"," ",$clientValue['dbday']);
                                                $nsex =  (!empty($clientValue['nsex']) && $clientValue['nsex'] == 2 ? 'Female' : 'Male' );
                                                $ccellarea = (is_array($clientValue['ccellarea']) ? '' : trim($clientValue['ccellarea']));
                                                $ccellphone = (is_array($clientValue['ccellphone']) ? '' : trim($clientValue['ccellphone']));
                                                $cbusarea = (is_array($clientValue['cbusarea']) ? '' : trim($clientValue['cbusarea']));
                                                $cbusphone = (is_array($clientValue['cbusphone']) ? '' : trim($clientValue['cbusphone']));
                                                //$irefid =  $clientValue['irefid'];
                                                //$irefperid =  $clientValue['irefperid'];

                                                $comparearray = array();

                                                $comparearray['Email'] = $cemail;
                                                $comparearray['Zip'] = $czip;
                                                $comparearray['Phone'] = $carea.$chomephone;
                                                $comparearray['Name'] = $cfirstname.' '.$clastname;
                                                $comparearray['Dob'] = $dbday;
                                                $comparearray['Sex'] = $nsex;
                                                $comparearray['Mobile'] = $ccellphone;
                                                //$comparearray['Mobile'] = '';
                                                $comparearray['MobileAreaCode'] = $ccellarea;
                                                $comparearray['BusinessAreaCode'] = $cbusarea;
                                                $comparearray['BusinessPhoneNumber'] = $cbusphone;
                                                $comparearray['clientFirstVistedDate'] = $firstVisitDate;
                                                $comparearray['clientLastVistedDate'] = $lastVisitDate;
                                                /*$comparearray['opted_in_email'] = 0;
                                                $comparearray['opted_in_sms'] = 0;

                                                $comparearray['HasMemberships'] = 0;
                                                $comparearray['ReferralTypeId'] = 0;
                                                $comparearray['ReferredByClientId'] = 0;*/
                                                ...
                                                if(!empty($arrClientsdata))
                                                    {
                                                       ...
                                                       if(empty($diff_array))
                                                        ...
                                                        }else
                                                        {
                                                            ...
                                                            $whereconditions['ClientId'] = $clientValue['iid'];
                                                           ...
                                                           /* $statusUpdate = "Updated Client Id ".$client_id.$hasEmail." and".$hasPhone." and Opted in email ".$optedInEMail." and ".$optedInSms." on ".date("Y-m-d H:i:s")."\n";
                                                            ...
                                                            $comparearray['ClientId'] =   $clientValue['iid'];
