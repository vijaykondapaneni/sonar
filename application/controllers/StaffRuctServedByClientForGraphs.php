...
class StaffRuctServedByClientForGraphs extends CI_Controller
{
    ...
    private function __getInsideSqlQueryFn($salonAccountNo, $temp)
    {
        ...
        $getUniqueClientTickets = $this->DB_ReadOnly->query("SELECT cinvoiceno
        ...
        $productSalesDetailsArrNew =  $this->DB_ReadOnly->select('count(DISTINCT cinvoiceno) as invoice_count,count(DISTINCT iclientid) as unique_client_count, SUM(nprice * nquantity) as nprice , SUM(nquantity) as nquantity')
        ...
        $serviceSalesDetailsArrNew =  $this->DB_ReadOnly->select('count(DISTINCT cinvoiceno) as invoice_count,count(DISTINCT iclientid) as unique_client_count,count(DISTINCT tdatetime) as tdatetime, SUM(nprice * nquantity) as nprice , SUM(nquantity) as nquantity')
                                                ->where($sqlCommonWeekCond)
                                                ->get_where(MILL_SERVICE_SALES,$whereServiceSalesCondition )
                                                ->row_array();
        
        //pa($this->db->last_query(),'$serviceSalesDetailsArrNew');
     ...
     * @param type $temp
     */    
    private function getRuctServedReports($temp)
    {
                            ...
                            foreach ($yearArr as $strYearType => $intYear)
                            {
                                foreach ($daterangeDateObj as $key => $date)
                                    ...
                                    //Calculation the Ruct server by client for this and last year of current month  
                                     $tempArr = $this->__calculateRuctServedByClient($strYearType,'Yearly');
                                     
                            }
                            
                        $this->staffCalcData["ruct"] = $tempArr;
                    ...
                    case MONTHLY:
                            ...
                            foreach ($yearArr as $strYearType => $intYear)
                            {
                                ...
                                foreach ($fourWeeksArr as $key => $date)
                                   ...
                                   //pa( $this->insideFormulaCalcuation,' $this->insideFormulaCalcuation');
                                    //Calculation the prebook and rebook for this and last year of current month  
                                    $tempArr = $this->__calculateRuctServedByClient($strYearType,$this->dayRangeType);
                            }
    ...
    private function __calculateRuctServedByClient($strYearType,$Key = '')
    {
            ...
            if(!empty($this->insideFormulaCalcuation))
                {
                    ...
                    foreach($this->insideFormulaCalcuation as $month_week_name => $dayRanges)
                    {
                        ...
                        $this->__insideConfigArr['uniqueInvoicesArr'.$strYearType] = (isset($uniqueInvoiceArray['New'])) ? array_column($uniqueInvoiceArray['New'], "cinvoiceno") : array();
                        ...
                        //CLIENTS SERVED    
                        $this->_staffCalcData[$month_week_name][$strYear.'clients_served_value'] = (($this->__insideConfigArr['totalUniqueInvoices'.$strYearType] != 0) && isset($servicesArray['New']['tdatetime']) && !empty($servicesArray['New']['tdatetime']) ) ? $this->Common_model->appCloudNumberFormat($this->__insideConfigArr['totalUniqueInvoices'.$strYearType]/$servicesArray['New']['tdatetime'],2) : '0.00';


                        //RUCT
                         $this->_staffCalcData[$month_week_name][$strYear.'RUCT'] = (($this->__insideConfigArr['totalUniqueInvoices'.$strYearType] != 0) && isset($productsArray['New']['nquantity']) && !empty($productsArray['New']['nquantity']) ) ? $this->Common_model->appCloudNumberFormat($productsArray['New']['nquantity']/$this->__insideConfigArr['totalUniqueInvoices'.$strYearType],2) : '0.00';
                       //pa($strYearType.' '.$month_week_name. ' >>>>  '.$productsArray['New']['nquantity'] .' --- '.$this->__insideConfigArr['totalUniqueInvoices'.$strYearType])  ;
     ...
     */
    function setStaffRuctServedInternalGraph($dayRangeType="Today",$account_no="",$year= 1)
		{
			...
			if(isset($this->salonsInfo["mill_salons"]) && !empty($this->salonsInfo["mill_salons"]))
			{
				foreach($this->salonsInfo["mill_salons"] as $salonsData)
				{
                    ...
                    $log['whichCron'] = 'setStaffRuctServedInternalGraph';
					...
					if(isset($getAllStaff["getAllStaff"]) && !empty($getAllStaff["getAllStaff"]))
					{
						foreach($getAllStaff["getAllStaff"] as $staffMembers)
						{
                           ...
                           // GET CALCULATE VALEUES AS PER FORMULAS 
                            $this->getRuctServedReports($temps);
                           
                            if(isset($this->staffCalcData["ruct"]) && !empty($this->staffCalcData["ruct"]))
							{
								//JSON DECODE FOR EACH LOOP STARTS
								foreach($this->staffCalcData["ruct"] as $resValue)
								{
									...
									$insertDataArray["RUCT"] = $resValue["RUCT"];
									$insertDataArray["clients_served_value"] = $resValue["clients_served_value"];
									$insertDataArray["key"] = $resValue["key"];
									$insertDataArray["last_year_RUCT"] = $resValue["last_year_RUCT"];
									$insertDataArray["last_year_clients_served_value"] = $resValue["last_year_clients_served_value"];
									...
									$reportsDataForSalon = $this->DB_ReadOnly->select('*')
                                                                    ->get_where(MILL_RUCT_CALCULATION_CRON,$reportsWhere)
                                                                    ->row_array();
                                    ...
                                    if(!empty($reportsDataForSalon))
								    {
								    	...
								    	{
                                            //Update REPORT
								    		$insertDataArray["insert_status"] = StaffRuctServedByClientForGraphs::UPDATED;
											$insertDataArray["updatedDate"] = date("Y-m-d H:i:s");
								    		try {
											    $this->db->where('id', $reportsDataForSalon['id']);
									    		$this->db->update(MILL_RUCT_CALCULATION_CRON, $insertDataArray); 
								    ...
								    {
                                        	//INSERT REPORT
									    	$insertDataArray["insert_status"] = StaffRuctServedByClientForGraphs::INSERTED;
											$insertDataArray["insertedDate"] = date("Y-m-d H:i:s");
											$insertDataArray["updatedDate"] = date("Y-m-d H:i:s");
											try {
												$this->db->insert(MILL_RUCT_CALCULATION_CRON, $insertDataArray);
