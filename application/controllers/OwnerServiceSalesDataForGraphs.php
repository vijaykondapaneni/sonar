...
class OwnerServiceSalesDataForGraphs extends CI_Controller
{
    /**
    AUTHOR: Subbu
    ...
    **/
                ...
                if(($dayRangeType=="Yearly")||($dayRangeType=="Previousmonths") ){
                    ...
                    foreach ($daterange as $key => $date) {
                        ...
                        $serviceSalesDetailsArr = $this->GraphsOwner_model
                                                  ...
                                                  ->row_array();
                        $salesArray[$monthName]['Services']['service'] = !empty($serviceSalesDetailsArr)? $serviceSalesDetailsArr : array();
                       ...
                       $year = $lastyear -1;
                        ...
                        $serviceSalesDetailsArrLastyear = $this->GraphsOwner_model
                                                  ->getServiceSalesDetailsArr($whereConditions,$where)
                                                  ->row_array();
                        $salesArray[$monthName]['last_year_Services']['service'] = !empty($serviceSalesDetailsArrLastyear)? $serviceSalesDetailsArrLastyear : array();
                    ...
                    if(!empty($salesArray)){
                        ...
                        foreach ($salesArray as $month_week_name => $dayRanges) {
                           ...
                           $totalSales['current_value'] = isset($servicesArray['service']['nprice']) ? $this->Common_model->appCloudNumberFormat($servicesArray['service']['nprice'], 2):'0';
                           $totalSales['key'] = substr($month_week_name, 0, 3);
                           $totalSales['last_year_value'] = isset($lastYearServicesArray['service']['nprice']) ? $this->Common_model->appCloudNumberFormat($lastYearServicesArray['service']['nprice'], 2):'0';
                    ...
                    $response["status"] = true;
                        ...
                        $salonAccountNo = $salonDetails['salon_account_id'];
                        $dataArray['location_id'] = $salon_id;
                        $dataArray['location_name'] = $salonDetails['salon_name'];
                        ...
                        foreach ($fourWeeksArr as $dates) {
                            ...
                            $serviceSalesDetailsArr = $this->GraphsOwner_model
                                                      ->getServiceSalesDetailsArr($whereConditions,$where)
                                                      ->row_array();
                            $salesArray[$week_number_db]['Services']['service'] = !empty($serviceSalesDetailsArr)? $serviceSalesDetailsArr : array();
                           ...
                           // pa($fourWeeksArrLastYear,'fourWeeksArrLastYear',false);
                            ...
                            pa($startDayOfWeekLastYear." To ".$endDayOfWeekLastYear,'start and end dates of the last year',false);
                            /*$where = "MONTH(tdatetime) = $month AND YEAR(tdatetime) =$year AND WEEK(tdatetime) =$week";*/
                            ...
                            $serviceSalesDetailsArrLastyear = $this->GraphsOwner_model
                                                      ->getServiceSalesDetailsArr($whereConditions,$where)
                                                      ->row_array();
                            //pa($serviceSalesDetailsArrLastyear);                          
                            $salesArray[$week_number_db]['last_year_Services']['service'] = !empty($serviceSalesDetailsArrLastyear)? $serviceSalesDetailsArrLastyear : array(); 
                            $week_number++;
                            $week_number_db++;
                        }
                        if(!empty($salesArray))
     ...
     */
    function setOwnerServiceSalesDataForGraphs($dayRangeType="today",$salon_id="")
        { 
                ...
                {   
                    ...
                    if(!in_array($salon_id, $processedSalons))
                    {
                     ...
                     if(!empty($salonDetails)&& isset($salonDetails['salon_info']["millennium_enabled"]) && isset($salonDetails['salon_info']["service_retail_reports_enabled"]) && $salonDetails['salon_info']["millennium_enabled"]=="Yes" && $salonDetails['salon_info']["service_retail_reports_enabled"]=="Yes"){
                        ...
                        $log['whichCron'] = 'setOwnerServiceSalesDataForGraphs';
                        $log['CronUrl'] = MAIN_SERVER_URL.'OwnerServiceSalesDataForGraphs/setOwnerServiceSalesDataForGraphs/'.$dayRangeType.'/'.$salon_id;
                        $log['CronType'] = 1;
                        $log['id'] = 0;
                                    ...
                                    foreach($resValue['graph_data'] as $resValuess){
                                       ...
                                       $monthRes = (new DateTime($this->endDate))->format("m");
                                        ...
                                        $reportsDataForSalon = $this->GraphsOwner_model
                                                               ->compareServiceOwnerReportsData($reportsWhere)
                                                               ->row_array();
                                        ...
                                        if(!empty($reportsDataForSalon)){
                                             ...
                                             if(empty($diff_array)){
                                              pa('No Updates');
                                                ...
                                                // update
                                                $diff_array['insert_status'] = OwnerServiceSalesDataForGraphs::UPDATED;
                                                ...
                                                try {
                                                    ...
                                                    $update = $this->GraphsOwner_model->updateServiceOwnerReportsData($reportsWhere,$diff_array);
                                              ...
                                              // Insert Data
                                                $dataArray["insert_status"] = OwnerServiceSalesDataForGraphs::INSERTED;
                                                $dataArray["insertedDate"] = date("Y-m-d H:i:s");
                                                $dataArray["updatedDate"] = date("Y-m-d H:i:s");

                                                try {
                                                    //pa($dataArray,'Insert');
                                                    $insert = $this->GraphsOwner_model->insertServiceOwnerReportsData($dataArray);
