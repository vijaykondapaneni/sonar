...
class Wsimport_all_salon_services_data_today extends CI_Controller {
			...
			if($getConfigDetails->num_rows()>0)
			{
				foreach($getConfigDetails->result_array() as $configDetails)
				{
					pa('',"".$configDetails['salon_name'].' ---['.$configDetails['salon_account_id']."]");
                     ...
                     $log['CronUrl'] = MAIN_SERVER_URL.'Wsimport_all_salon_services_data_today/GetServiceSales/'.$salon_code_encode;
                     $log['CronType'] = 0;
                     $log['id'] = 0;
