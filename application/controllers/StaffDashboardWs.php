    ...
    public function getStaffDashboardWs($dayRangeType="Today"){
        ...
        if(isset($_POST['salon_id']) && !empty($_POST['salon_id'])){
                if(!isset($_POST['staff_id']) && empty($_POST['staff_id'])) {
                    ...
                    $last_updated = !empty($res['updatedDate']) ? date("m/d/Y H:i:s", strtotime($res['updatedDate'])) : ""; 
                    $tempResponseArr['last_updated'] = $last_updated;   
                    $icon_empty = 'https://s3.amazonaws.com/appreportsicons/PiggyBank_Dead.png';
                    $icon = 'https://s3.amazonaws.com/appreportsicons/money+bag.png';
                    if($this->responseArr["total_retail_price"] + $this->responseArr["total_service_sales"] > 0){
                     $icon_display = $icon;
                    }else{
                     $icon_display = $icon_empty;
                    }
