...
class OwnerRepeatedGuestDataForGraphsLastYear extends CI_Controller
{
    /**
    AUTHOR: Subbu
    ...
    **/
                    ...
                    if($dayRangeType=="Yearly"){
                        ...
                        foreach ($relatedSalons as $eachSalon) {
                            ...
                            foreach ($daterange as $key => $date) {
                                ...
                                $salesArray[$monthName]['dates']['checked_in'][0]['start_date'] = $firstDayOfMonth;
                                $salesArray[$monthName]['dates']['checked_in'][0]['end_date'] = $lastdayOfMonth; 
                            }
                            if(!empty($salesArray)){
                                $tempArr = array();
                                foreach ($salesArray as $month_week_name => $dayRanges) {
                                   ...
                                   $totalSales["start_date"] = $datesArray['checked_in'][0]['start_date'];
                                   $totalSales["end_date"] = $datesArray['checked_in'][0]['end_date'];
     ...
     */
    function setOwnerRepeatedGuestDataForGraphsLastYear($dayRangeType="today",$year=1,$salon_id="")
        { 
                ...
                foreach($getAllSalons["mill_salons"] as $salonsData)
                {
                    ...
                    if(!in_array($salon_id, $processedSalons))
                    {
                        ...
                        if(!empty($salonDetails)&& isset($salonDetails['salon_info']["millennium_enabled"]) && $salonDetails['salon_info']["millennium_enabled"]=="Yes"){
                            ...
                            $log['whichCron'] = 'setOwnerRepeatedGuestDataForGraphsLastYear';
                            ...
                            foreach($getAllRetailData['values'] as $resValue){
                                ...
                                if(isset($resValue["graph_data"]) && !empty($resValue["graph_data"]))
                                {
                                    foreach($resValue['graph_data'] as $resValuess){
                                        ...
                                        // compare database
                                        $reportsWhere = array('salon_id' => $salonId,'dayRangeType' => $dayRangeType,'key' => $resValuess['key'],'start_date' => $startDate,'end_date' =>$endDate);
                                        ...
                                        if(!empty($reportsDataForSalon)){
                                             ...
                                             if(empty($diff_array)){
                                               pa('No Updates');
                                                ...
                                                // update
                                                $diff_array['insert_status'] = OwnerRepeatedGuestDataForGraphsLastYear::UPDATED;
                                              ...
                                              // Insert Data
                                                $dataArray["insert_status"] = OwnerRepeatedGuestDataForGraphsLastYear::INSERTED;
