...
class OwnerClientServicedDataForGraphs extends CI_Controller
{
    /**
    AUTHOR: Subbu
    ...
    **/
       ...
       if(!empty($data["salon_id"]) && !empty($data["start_date"]) && !empty($data["end_date"])&& !empty($data["dayRangeType"]))
            {
                ...
                if(($dayRangeType=="Yearly") || ($dayRangeType=="Previousmonths")){
                    ...
                    foreach ($daterange as $key => $date) {
                      ...
                      //  pa($salonDetails,'salonDetails');
                        //pa($salonAccountNo."subbu",'salonAccountNo');
                        ...
                        $prebookArray[$monthName]['dates']['UniqueClient']['start_date'] = $firstDayOfMonth;
                        $prebookArray[$monthName]['dates']['UniqueClient']['end_date'] = $lastdayOfMonth; 
                    ...
                    if(!empty($prebookArray)){
                            ...
                            foreach ($prebookArray as $month_week_name => $dayRanges) {
                               ...
                               $prebookPercentage['current_value'] = !empty($totalUniqueInvoices) && isset($servicesArray['New']['tdatetime']) && !empty($servicesArray['New']['tdatetime']) ? $this->Common_model->appCloudNumberFormat($totalUniqueInvoices/$servicesArray['New']['tdatetime'], 2):'0';
                               $prebookPercentage['key'] = substr($month_week_name, 0, 3);

                               $uniqueInvoicesArrLastYear = isset($lastYearuniqueInvoiceArray['New']) ? array_column($lastYearuniqueInvoiceArray['New'], "cinvoiceno") : array();

                               $totalUniqueInvoices = !empty($uniqueInvoicesArrLastYear) ? count($uniqueInvoicesArrLastYear) : 0;


                               $prebookPercentage['last_year_value'] = !empty($totalUniqueInvoices) && isset($lastYearServicesArray['New']['tdatetime']) && !empty($lastYearServicesArray['New']['tdatetime']) ? $this->Common_model->appCloudNumberFormat($totalUniqueInvoices/$lastYearServicesArray['New']['tdatetime'], 2):'0';


                               $prebookPercentage["start_date"] = $datesArray['UniqueClient']['start_date'];
                               $prebookPercentage["end_date"] = $datesArray['UniqueClient']['end_date'];
                    ...
                    foreach ($fourWeeksArr as $dates) {
                        ...
                        $getUniqueClientTickets = $this->GraphsOwner_model
                                                  ->getUnqiueClientsWeekWise($salonAccountNo,$startDayOfWeek,$endDayOfWeek)
                                                  ->result_array();
                                                

                        $prebookArray[$week_number_db]['Both']['New'] = !empty($getUniqueClientTickets) ? $getUniqueClientTickets : array();
                        ...
                        $serviceSalesDetailsArrNew = $this->GraphsOwner_model
                                                  ->getServiceSalesData($whereConditions,$where)
                                                  ->row_array();
                        $prebookArray[$week_number_db]['Services']['New'] = !empty($serviceSalesDetailsArrNew)? $serviceSalesDetailsArrNew : array();
                       ...
                       //LAST Year Same MONTH DATA
                        ...
                        $getUniqueClientTicketsLastyear = $this->GraphsOwner_model
                                                          ->getUnqiueClientsWeekWise($salonAccountNo,$startDayOfWeekLastYear,$endDayOfWeekLastYear)
                                                          ->result_array();

                        $prebookArray[$week_number_db]['last_year_Both']['New'] = empty($getUniqueClientTicketsLastyear) ? $getUniqueClientTicketsLastyear:array();                                    
                        ...
                        $prebookArray[$week_number_db]['last_year_Services']['New'] = !empty($serviceSalesDetailsArrNewLastyear)? $serviceSalesDetailsArrNewLastyear : array();                          
                    ...
                    if(!empty($prebookArray))
                    {
                        ...
                        foreach($prebookArray as $month_week_name => $dayRanges)
                        {
                            ...
                            if(isset($uniqueInvoiceArray['New']))
                            {
                                $uniqueInvoicesArr = array_column($uniqueInvoiceArray['New'], "cinvoiceno");
                                $totalUniqueInvoices = count($uniqueInvoicesArr);
                            }
                            else
                            {
                                $uniqueInvoicesArr = array();
                                $totalUniqueInvoices = 0;
                            }
                            //echo $totalUniqueInvoices;exit;
                            
                            //CLIENTS SERVED
                            if(!empty($totalUniqueInvoices) && isset($servicesArray['New']['tdatetime']) && !empty($servicesArray['New']['tdatetime']))
                            {
                                $prebookPercentage["current_value"] = number_format($totalUniqueInvoices/$servicesArray['New']['tdatetime'], 2, '.', '');
                            ...
                            if(isset($lastYearuniqueInvoiceArray['New']))
                            {
                                $uniqueInvoicesArr = array_column($lastYearuniqueInvoiceArray['New'], "cinvoiceno");
                                $totalUniqueInvoices = count($uniqueInvoicesArr);
                            }
                            else
                            {
                            ...
                            //CLIENTS SERVED
                            if(!empty($totalUniqueInvoices) && isset($lastYearServicesArray['New']['tdatetime']) && !empty($lastYearServicesArray['New']['tdatetime']))
                            {
                                $prebookPercentage["last_year_value"] = number_format($totalUniqueInvoices/$lastYearServicesArray['New']['tdatetime'], 2, '.', '');
     ...
     */
    function setOwnerClientServicedDataForGraphs($dayRangeType="today",$salon_id="")
        { 
            ...
            if(isset($getAllSalons["mill_salons"]) && !empty($getAllSalons["mill_salons"]))
            {
                ...
                {
                    //pa($salonDetails['salon_info'],'salonsData',true);
                    ...
                    if(!in_array($this->salon_id, $processedSalons))
                    {
                      if(!empty($salonDetails)&& isset($salonDetails['salon_info']["millennium_enabled"]) && $salonDetails['salon_info']["millennium_enabled"]=="Yes"){
                        ...
                        $log['whichCron'] = 'setOwnerClientServicedDataForGraphs';
                        $log['CronUrl'] = MAIN_SERVER_URL.'OwnerClientServicedDataForGraphs/setOwnerClientServicedDataForGraphs/'.$dayRangeType.'/'.$salon_id;
                                        ...
                                        if(!empty($reportsDataForSalon)){
                                             ...
                                             if(empty($diff_array)){
                                               pa('No Updates'); 
                                                ...
                                                // update
                                                $diff_array['insert_status'] = OwnerClientServicedDataForGraphs::UPDATED;
                                              ...
                                              // Insert Data
                                                $dataArray["insert_status"] = OwnerClientServicedDataForGraphs::INSERTED;
